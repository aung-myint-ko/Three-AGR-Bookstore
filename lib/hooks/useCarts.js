...
import { jwtDecode, jwtEncode } from "./useJwt";
...
const useCarts = () => {
  ...
  const encryptedCart = jwtEncode(cart);
  const encryptedCart2 = jwtDecode(encryptedCart);
  const decodeCart = jwtDecode(encryptedCart2.id);
