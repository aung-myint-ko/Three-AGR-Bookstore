import { useJwtDecode, useJwtEncode } from "@/lib-hook/useJwt";
...
const addCartItem = (state, product) => {
  let filterCart = [],
    sameProduct;
  const originalCart = useJwtDecode(state.cart);
...
const removeCartItem = (state, id) => {
  ...
  const encodeCart = useJwtEncode([...filterCart]);
...
const increaseCartItem = (state, id) => {
  ...
  const encodeCart = useJwtEncode(updatedCart);
